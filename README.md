...
- 新闻页支持下拉刷新、关键词搜索、平台分类、范围标签筛选、详情阅读和已读/未读标记。
...
```bash
./gradlew :app:assembleDebug --stacktrace -PVERSION_NAME=1.0.0 -PVERSION_CODE=10000
```
...
本地验证在线升级时，应使用同一签名链路。例如当前模拟器安装的是 debug 版，则测试 Release 中的新版 APK 也应使用本机 debug 签名：

```bash
./gradlew :app:assembleDebug --stacktrace -PVERSION_NAME=1.0.0 -PVERSION_CODE=10000
gh release create v1.0.0 app/build/outputs/apk/debug/app-debug.apk#HotNews-v1.0.0-debug.apk \
  --title "HotNews v1.0.0" \
